...
import type { Note, TSPProject } from '@/types'
...
export default function TSPNotesPage({ onNavigate, onSwitchWorkspace, initialNoteId }: Props) {
  const [tspProjects, setTspProjects] = useState<TSPProject[]>([])
  ...
  const load = useCallback(async () => {
    ...
    setTspProjects(store.tspProjects ?? [])
    setNotes(store.tspNotes ?? [])
  }, [])
  ...
  useEffect(() => {
    return () => {
      if (saveTimerRef.current) {
        ...
        if (draftRef.current) {
          window.api.updateTSPNote(draftRef.current.id, draftRef.current)
        }
      }
    }
  }, [])
  ...
  function scheduleSave(updated: Note) {
    ...
    saveTimerRef.current = setTimeout(async () => {
      try {
        const saved = await window.api.updateTSPNote(updated.id, updated)
  ...
  async function flushSave() {
    if (saveTimerRef.current && draftRef.current) {
      ...
      const saved = await window.api.updateTSPNote(draftRef.current.id, draftRef.current)
      setNotes((prev) => prev.map((n) => (n.id === saved.id ? saved : n)))
    }
  }
  ...
  async function deleteNote() {
    if (!draft) return
    if (saveTimerRef.current) clearTimeout(saveTimerRef.current)
    await window.api.deleteTSPNote(draft.id)
                ...
                {draft.checklistItems.map((item) => (
                  <div key={item.id} className="flex items-center gap-2.5 group">
                    ...
                    <input
                      ...
                      onKeyDown={(e) => {
                        if (e.key === 'Enter') { e.preventDefault(); addChecklistItem(item.id) }
                        else if (e.key === 'Backspace' && item.text === '') { e.preventDefault(); deleteChecklistItem(item.id) }
                      }}
                  ...
                  return (
                    <button
                      ...
                      className={`flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs transition-colors cursor-pointer ${
                        linked ? 'bg-white/20 text-white' : 'bg-white/5 text-white/35 hover:bg-white/10 hover:text-white/60'
                      }`}
                    >
            ...
            <div className="mt-8 pt-4 border-t border-white/10">
              {showDeleteConfirm ? (
                <div className="flex items-center gap-3">
                  ...
                  <button onClick={deleteNote} className="text-sm text-red-400 hover:text-red-300 transition-colors cursor-pointer font-medium">
                    Yes, delete
                  </button>
                  <button onClick={() => setShowDeleteConfirm(false)} className="text-sm text-white/40 hover:text-white/70 transition-colors cursor-pointer">
                    Cancel
                  </button>
                </div>
              ) : (
                <button onClick={() => setShowDeleteConfirm(true)} className="text-sm text-white/25 hover:text-red-400 transition-colors cursor-pointer">
                  Delete note
                </button>
              )}
            </div>
